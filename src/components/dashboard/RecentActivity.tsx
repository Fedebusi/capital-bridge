...
const activities = [
  ...
  { icon: FileText, color: "text-slate-500", bg: "bg-muted", label: "DD report generated — Palau de Gràcia", time: "2d ago" },
  { icon: Clock, color: "text-slate-500", bg: "bg-muted", label: "New deal received — Mirador del Port", time: "3d ago" },
];

export default function RecentActivity() {
  return (
    <div className="rounded-2xl bg-slate-50 p-6">
      <h3 className="font-display text-sm font-semibold text-primary mb-4">Recent Activity</h3>
        ...
        {activities.map((a, i) => (
          <div key={i} className="flex items-start gap-3 animate-fade-in" style={{ animationDelay: `${i * 60}ms` }}>
            ...
            <div className="flex-1 min-w-0">
              <p className="text-sm text-primary leading-snug">{a.label}</p>

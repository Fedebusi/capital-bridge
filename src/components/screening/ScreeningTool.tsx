...
export default function ScreeningTool() {
  ...
  const handleScreen = () => {
    ...
    const screeningResults: ScreeningResult[] = [
      {
        label: "Asset Type",
        pass: defaultCriteria.acceptedAssetTypes.includes(formData.assetType),
        value: formData.assetType || "Not specified",
        threshold: "Residential",
      },
      {
        label: "Location",
        pass: formData.location !== "",
        value: formData.location || "Not specified",
        threshold: "Spain/Portugal Tier 1-2",
      },
      {
        label: "LTV at Origination",
        pass: ltv > 0 && ltv <= defaultCriteria.maxLTV,
        value: ltv > 0 ? `${ltv.toFixed(1)}%` : "N/A",
        threshold: `≤ ${defaultCriteria.maxLTV}%`,
      },
      {
        label: "LTC",
        pass: ltc > 0 && ltc <= defaultCriteria.maxLTC,
        value: ltc > 0 ? `${ltc.toFixed(1)}%` : "N/A",
        threshold: `≤ ${defaultCriteria.maxLTC}%`,
      },
      {
        label: "Ticket Size",
        pass: loanAmount >= defaultCriteria.minTicket && loanAmount <= defaultCriteria.maxTicket,
        value: loanAmount > 0 ? `€${(loanAmount / 1000000).toFixed(1)}M` : "N/A",
        threshold: `€${defaultCriteria.minTicket / 1000000}M - €${defaultCriteria.maxTicket / 1000000}M`,
      },
      {
        label: "Developer Track Record",
        pass: devProjects >= defaultCriteria.minDeveloperProjects,
        value: `${devProjects} projects`,
        threshold: `≥ ${defaultCriteria.minDeveloperProjects} projects`,
      },
      {
        label: "Pre-Sales Level",
        pass: preSales >= defaultCriteria.minPreSales,
        value: `${preSales}%`,
        threshold: `≥ ${defaultCriteria.minPreSales}%`,
      },
    ];
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        ...
        <div className="rounded-xl border border-border bg-card p-6 shadow-card">
          ...
          <p className="text-sm text-muted-foreground mb-6">
            Input basic deal parameters to get an instant pass/fail assessment against fund investment criteria.
          </p>
          ...
          <div className="space-y-4">
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <Input
                  ...
                  placeholder="e.g. Residencial Sol"
              ...
              <div>
                ...
                <Input
                  ...
                  onChange={e => setFormData(p => ({ ...p, borrower: e.target.value }))}
                  placeholder="e.g. Grupo XYZ"
                  className="mt-1 bg-muted border-border"
                />
              </div>
            </div>
            ...
            <div className="grid grid-cols-3 gap-4">
              ...
              <div>
                ...
                <Input
                  type="number"
                  value={formData.gdv}
                  onChange={e => setFormData(p => ({ ...p, gdv: e.target.value }))}
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <Input
                  type="number"
                  value={formData.preSales}
                  onChange={e => setFormData(p => ({ ...p, preSales: e.target.value }))}
                  placeholder="25"
                  className="mt-1 bg-muted border-border"
                />
              </div>
              <div>
                ...
                <Input
                  type="number"
                  value={formData.developerProjects}
                  onChange={e => setFormData(p => ({ ...p, developerProjects: e.target.value }))}
                  placeholder="8"
                  className="mt-1 bg-muted border-border"
                />
              </div>
            ...
            <div className="flex gap-3 pt-2">
              <Button onClick={handleScreen} className="flex-1 gap-2">
                  ...
                  {results.map((r, i) => (
                    <motion.div
                      ...
                      className={cn(
                        ...
                        r.pass ? "border-success/20 bg-success/5" : "border-destructive/20 bg-destructive/5"
                      )}
                    >
